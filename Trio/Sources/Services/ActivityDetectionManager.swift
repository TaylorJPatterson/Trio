...
final class ActivityDetectionManager: Injectable {
    ...
    private let requiredValidations = 2
    // To make more sensitive: Change 3 to 2 to require fewer validations before triggering.
    ...
    private func detectPrimaryActivity(from activity: CMMotionActivity) -> ActivityType? {
        ...
        // To make more sensitive: Change .high to .medium
        guard activity.confidence == .medium else {
            return nil
        }
    ...
    private func handleActivityDetected(_ activity: ActivityType, confidence _: CMMotionActivityConfidence) {
        ...
        if currentActivity == activity {
            ...
            // Start validation timer to check every X seconds.
            // To make more sensitive: Change 20.0 to 15.0 to check activity more frequently.
            DispatchQueue.main.async { [weak self] in
    ...
    private func validateContinuousActivity() {
        ...
        // Query recent activity to ensure it's still happening. To make more sensitive: Change -30 to -20 to use a shorter time window for validation.
        motionManager
